...
# -------------------- Summary (NEW×FAILED and RESOLVED×NOT_FAILED) --------------------
def build_workflow_compliance_summary_new_failed_and_resolved_not_failed(findings):
    """
    Count unique resources for just:
      - NEW × FAILED
      - RESOLVED × NOT_FAILED (i.e., any compliance status except FAILED)
    """
    combos = {("NEW","FAILED"): 0, ("RESOLVED","NOT_FAILED"): 0}
    seen = set()  # uniqueness key: (accountId, resourceId, workflow, compliance_group)

    for f in findings:
        wf = (f.get("Workflow", {}) or {}).get("Status", "") or ""
        cp = (f.get("Compliance", {}) or {}).get("Status", "") or ""

        if wf == "NEW" and cp == "FAILED":
            target_key = ("NEW","FAILED")
        elif wf == "RESOLVED" and cp and cp != "FAILED":
            target_key = ("RESOLVED","NOT_FAILED")
        else:
            continue
        ...
        uniq = (acct, rid, target_key[0], target_key[1])
        if uniq not in seen and (acct or rid):
            combos[target_key] += 1
            seen.add(uniq)
...
def explanations_html(counts) -> str:
    new_failed   = counts.get(("NEW","FAILED"), 0)
    res_notfail  = counts.get(("RESOLVED","NOT_FAILED"), 0)
    items = []
    if new_failed:
        items.append(f'<li><b>NEW × FAILED</b> — {new_failed} unique resources (newly opened, non-compliant).</li>')
    if res_notfail:
        items.append(f'<li><b>RESOLVED × NOT_FAILED</b> — {res_notfail} unique resources (closed and compliant/non-failed with a recent note update).</li>')
    if not items:
        return ""
...
def lambda_handler(event, context):
    ...
    # --- Slice B: Title starts with ... AND Compliance != FAILED AND Workflow=RESOLVED
    #               AND NoteUpdatedAt in last days_back AND NoteUpdatedBy present
    baseB = make_time_filter(days_back)
    optB  = make_optional_filters_title_prefix(
        rule_title_prefix=title_prefix,
        workflow_statuses=["RESOLVED"]
    )
    # Key fix: NOT_EQUALS FAILED (covers PASSED, WARNING, NOT_AVAILABLE, etc.)
    optB["ComplianceStatus"] = [{"Value": "FAILED", "Comparison": "NOT_EQUALS"}]

    noteB = make_note_filters(days_back=days_back, require_updated_by=True)
    filtersB = merge_filters(merge_filters(baseB, optB), noteB)
    logger.info("Filters B (RESOLVED×NOT_FAILED with recent note): %s", json.dumps(filtersB))
    ...
    counts = build_workflow_compliance_summary_new_failed_and_resolved_not_failed(findings)
    ...
    email_body = os.getenv(
        "EMAIL_BODY",
        f"BMOASR Security Hub Findings in the last {days_back} days (NEW×FAILED and RESOLVED×NOT_FAILED)."
    )
    ...
    return {
        ...
        "body": json.dumps({
            ...
            "rows_slice_B_resolved_not_failed_note_recent": len(findingsB),
