...
Description: >
  ...
  (Parse → Branch → Describe → Act → Resolve → Re-Evaluate). Keep DryRun as STRING to avoid SSM boolean/substring errors.
...
Resources:
            ...
            Statement:
              ...
              - Effect: Allow
                Action:
                  ...
                  - config:DescribeConfigRuleEvaluationStatus   # optional (if polling later)
                Resource: "*"
  ...
  ########################################
  # 3) SSM Automation (Security Hub → Describe/Branch → Act → Resolve → Re-Evaluate)
  ########################################
    ...
    Properties:
      ...
      Content:
        ...
        description: !Sub >
          ...
          then RESOLVE the finding for ${Team}/${Purpose}. Keep DryRun as STRING. Finally, trigger a Config re-evaluation.
        ...
        parameters:
          ...
          # === New: which rule to re-evaluate + small delay
          ConfigRuleName:
            type: String
            default: !Sub "OMBASR-Configrule-${Team}-${Service}${Purpose}"
            description: "Config rule to re-evaluate after remediation."
          ReevalDelaySeconds:
            type: String
            default: "20"
            description: "Wait so Config ingests tag change before re-evaluation."
        ...
        mainSteps:
          ...
          # 8) Give Config a moment to ingest the tag change
          - name: SleepBeforeReeval
            action: aws:sleep
            nextStep: ReEvaluateConfigRule
            inputs:
              DurationSeconds: "{{ ReevalDelaySeconds }}"

          # 9) Trigger on-demand re-evaluation of the rule
          - name: ReEvaluateConfigRule
            description: "Force an on-demand evaluation of the specified Config rule."
            action: aws:executeAwsApi
            isEnd: true
            inputs:
              Service: config
